...
class XHProfRuns_Default implements iXHProfRuns
{
  ...
  public function getRuns($stats)
  {
    ...
    if (isset($stats['select'])) {
      $query = "SELECT {$stats['select']} FROM `{$_xhprof['dbtable']}` ";
    } else {
      $query = "SELECT * FROM `{$_xhprof['dbtable']}` ";
    }
  ...
  public function getDistinct($data)
  {
    ...
    $query = "SELECT DISTINCT(`{$sql['column']}`) FROM `{$_xhprof['dbtable']}`";
  ...
  public function get_run($run_id, $type, &$run_desc)
  {
    ...
    $query = "SELECT * FROM `{$_xhprof['dbtable']}` WHERE `id` = '$run_id'";
    $resultSet = $this->db->query($query);
  ...
  public function getRunComparativeData($url, $c_url)
  {
    ...
    $query = "SELECT count(`id`), avg(`wt`), min(`wt`), max(`wt`),  avg(`cpu`), min(`cpu`), max(`cpu`), avg(`pmu`), min(`pmu`), max(`pmu`) FROM `{$_xhprof['dbtable']}` WHERE `url` = '$url'";
    $rs = $this->db->query($query);
    ...
    //Runs same c_url
    //  count, avg/min/max for wt, cpu, pmu
    $query = "SELECT count(`id`), avg(`wt`), min(`wt`), max(`wt`),  avg(`cpu`), min(`cpu`), max(`cpu`), avg(`pmu`), min(`pmu`), max(`pmu`) FROM `{$_xhprof['dbtable']}` WHERE `c_url` = '$c_url'";
    $rs = $this->db->query($query);
    $row = $this->db->getNextAssoc($rs);
    $row['url'] = $c_url;
  ...
  protected function calculatePercentile($details)
  {
    ...
    $query = "SELECT `{$details['column']}` as `value` FROM `{$_xhprof['dbtable']}` WHERE `{$details['type']}` = '{$details['url']}' ORDER BY `{$details['column']}` DESC LIMIT $limit, 1";
    $rs = $this->db->query($query);
    $row = $this->db->getNextAssoc($rs);
    return $row['value'];
  }
